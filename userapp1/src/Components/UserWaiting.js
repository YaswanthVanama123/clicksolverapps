...
const WaitingUser = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.BACKENDAIPG}/api/workers-nearby`,
        {
          area,
          city,
          pincode,
  ...
  const handleManualCancel = async () => {
    try {
      if (decodedId) {
        await axios.post(`${process.env.BACKENDAIPG}/api/user/cancellation`, {
          user_notification_id: decodedId,
        });

        const cs_token = await EncryptedStorage.getItem('cs_token');
        await axios.post(
          `${process.env.BACKENDAIPG}/api/user/action/cancel`,
    ...
    const checkStatus = async () => {
      try {
        const response = await axios.get(
          `${process.env.BACKENDAIPG}/api/checking/status`,
          {
