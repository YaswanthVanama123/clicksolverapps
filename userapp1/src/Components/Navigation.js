...
const Navigation = () => {
  ...
  const handleCancelBooking = useCallback(async () => {
    ...
    try {
      const response = await axios.post(
        `${process.env.BACKENDAIPG}/api/user/work/cancel`,
      ...
      if (response.status === 200) {
        ...
        await axios.post(
          `${process.env.BACKENDAIPG}/api/user/action`,
          {
    ...
    const checkVerificationStatus = async () => {
      try {
        const response = await axios.get(
          `${process.env.BACKENDAIPG}/api/worker/verification/status`,
  ...
  useEffect(() => {
    const fetchWorkerDetails = async () => {
      ...
      try {
        const response = await axios.post(
          `${process.env.BACKENDAIPG}/api/worker/navigation/details`,
  ...
  useEffect(() => {
    const fetchLocationDetails = async () => {
      try {
        const response = await axios.get(
          `${process.env.BACKENDAIPG}/api/user/location/navigation`,
          {params: {notification_id: decodedId}},
        );
