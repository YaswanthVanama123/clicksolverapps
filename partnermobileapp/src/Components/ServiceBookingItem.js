...
const ServiceBookingItem = () => {
  ...
  const getTimelineData = useMemo(() => {
    ...
    return statusKeys.map((statusKey, index) => ({
      ...
      iconColor: index <= currentStatusIndex || currentStatusIndex === -1 ? '#ff4500' : '#a1a1a1',
      lineColor: index <= currentStatusIndex || currentStatusIndex === -1 ? '#ff4500' : '#a1a1a1',
    }));
  }, [status]);
    ...
    const fetchBookings = async () => { 
      try { 
        const response = await axios.post(
          `${process.env.BackendAPI6}/api/service/booking/item/details`,
          { tracking_id }
        ); 
            ...
            {getTimelineData.map((item, index) => (
              <View key={index} style={styles.timelineItem}>
                ...
                <View style={styles.timelineTextContainer}>
                  ...
                  <Text style={styles.timelineTime}>{item.time ? item.time : 'Pending'}</Text>
                </View>
              </View>
            ))}
          </View>
