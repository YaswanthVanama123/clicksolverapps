...
const CashbackScreen1 = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post(`${process.env.BackendAPI6}/api/worker/pending/cashback`, {
          worker_id: worker_id
        });
        ...
        Alert.alert('Error', 'Failed to fetch cashback data. Please try again later.');
      }
    }; 
    fetchData();
  }, [worker_id]);
  ...
  const handlesubmit = async() => {
    ...
    try {
      await axios.post(`${process.env.BackendAPI6}/api/worker/cashback/payed`, {
        worker_id,
  ...
  const renderPaymentHistoryItem = ({ item }) => (
    <View style={styles.historyItem}>
      <View style={styles.iconContainer}>
        <MaterialCommunityIcons name={item.debited ? "arrow-top-right" : "arrow-bottom-left"} size={20} color="#ffffff" />
      </View>
      <View style={styles.historyDetails}>
