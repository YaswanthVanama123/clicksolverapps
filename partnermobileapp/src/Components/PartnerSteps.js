  ...
  const fetchStepStatuses = useCallback(async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.BackendAPI5}/api/onboarding/step-status`,
        {},
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.stepRow}>
          <Icon name={step1Status ? "check-circle" : "radio-button-unchecked"} size={24} color={step1Status ? "#1DA472" : "#9e9e9e"} />
      ...
      >
        <View style={styles.stepRow}>
          <Icon name={step2Status ? "check-circle" : "radio-button-unchecked"} size={24} color={step2Status ? "#1DA472" : "#9e9e9e"} />
          <View style={styles.stepContent}>
        ...
        {selectedStep === 2 && !step2Status && (
          <TouchableOpacity style={styles.proceedButton} onPress={() => navigation.push("SkillRegistration")}>
      ...
      >
        <View style={styles.stepRow}>
          <Icon name={step3Status ? "check-circle" : "radio-button-unchecked"} size={24} color={step3Status ? "#1DA472" : "#9e9e9e"} />
          <View style={styles.stepContent}>
            <Text style={styles.stepTitle}>STEP 3</Text>
            <Text style={styles.stepName}>Adding Banking Details</Text>
            <Text style={[styles.stepStatus, !step3Status && { color: '#ff4500' }]}>
          ...
          {step3Status && <Icon name="check" size={24} color="#1DA472" />}
            ...
            <View style={styles.bankDetailsContainer}>
              <TouchableOpacity style={styles.optionRow} onPress={() => setBankingOption('bankAccount')}>
                <Icon name={bankingOption === 'bankAccount' ? "radio-button-checked" : "radio-button-unchecked"} size={20} color="#000" />
                <Text style={styles.optionText}>Add bank account</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.optionRow} onPress={() => setBankingOption('upiId')}>
                <Icon name={bankingOption === 'upiId' ? "radio-button-checked" : "radio-button-unchecked"} size={20} color="#000" />
