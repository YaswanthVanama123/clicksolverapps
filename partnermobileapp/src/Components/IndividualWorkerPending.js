...
const IndividualWorkerPending = () => {
  ...
  const handleApproved = async () => {
    const response = await axios.post(
      `http:192.168.243.71:5000/api/worker/approved`,
      {
        workerId,
      },
    );
  ...
  const submitAllIssues = async () => {
    ...
    try {
      await axios.post(`http:192.168.243.71:5000/api/update/worker/issues`, {
        workerId,
        issues,
      });
  ...
  const applyStatusChange = async newStatus => {
    try {
      await axios.post(
        `http:192.168.243.71:5000/api/aprove/tracking/update/status`,
        {
          workerId,
          newStatus,
        },
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const response = await axios.post(
          `http:192.168.243.71:5000/api/individual/worker/pending/verification`,
          {workerId},
        );
