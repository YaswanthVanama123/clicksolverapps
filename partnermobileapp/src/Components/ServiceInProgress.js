...
const ServiceInProgressScreen = () => {
    ...
    const fetchBookings = async () => {
      try {
        const response = await axios.post(
          `${process.env.BackendAPI14}/api/worker/work/progress/details`,
          {
            decodedId,
          },
        );
  ...
  const handleServiceCompletion = async () => {
    ...
    try {
      ...
      await axios.post(`${process.env.BackendAPI14}/api/worker/update/status`, {
        decodedId,
  ...
  const applyStatusChange = async (serviceId, statusKey) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.BackendAPI14}/api/worker/working/status/updated`,
        {
          serviceName,
          statusKey,
          currentTime,
  ...
  const confirmWorkPlace = async () => {
    if (accept) {
      ...
      try {
        ...
        const response = await axios.post(
          `${process.env.BackendAPI14}/api/add/tracking`,
          {
