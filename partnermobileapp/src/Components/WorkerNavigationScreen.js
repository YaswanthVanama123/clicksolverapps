...
const WorkerNavigationScreen = () => {
  ...
  const phoneCall = async () => {
    try { 
      const response = await axios.post('http://192.168.55.104:5000/api/user/call', { decodedId });
  ...
  const fetchAddressDetails = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://192.168.55.104:5000/api/user/address/details`,
        {params: {notification_id: decodedId}},
      );
  ...
  const fetchLocationDetails = async () => {
    try {
      const response = await axios.post(
        `http://192.168.55.104:5000/api/service/location/navigation`,
  ...
  const handleCancelBooking = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await axios.post(
        `http://192.168.55.104:5000/api/worker/work/cancel`,
        {notification_id: decodedId},
      );
