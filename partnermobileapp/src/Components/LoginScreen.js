...
const LoginScreen = () => {
 ...
 // Destructuring for cleaner code
  ...
  const loginBackend = async (phoneNumber) => {
    try {
      const response = await axios.post(`${process.env.BackendAPI5}/api/worker/login`, { phone_number: phoneNumber });
      return response;
    } catch (error) {
  ...
  const login = async () => {
    ...
    try {
      ...
      if (status === 200) {
      ...
      } else if (status === 201) { // Updated to 201
        ...
        if (phone_number) {
          ...
          console.error('Missing phone_number in response data for error status');
        }
      }
